...
class GPSWaypointManager:
    def __init__(self, waypoints_file_name, rcl_logger):
        ...
        """
        self.waypoints = GPSWaypointManager.parse_waypoints(waypoints_file_name)
    ...
    @staticmethod
    def parse_waypoints(waypoints_file_name: str) -> list[GPSCoordinate]:
        ...
        for waypoint in waypoints_data:
            lat = waypoint["lat"]
            lon = waypoint["lon"]
            waypoints.append(GPSCoordinate(lat, lon))

        return waypoints

    def get_next_waypoint(self) -> GPSCoordinate:
